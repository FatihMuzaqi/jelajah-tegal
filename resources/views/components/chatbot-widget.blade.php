...
<div id="jelajah-chatbot-root">
    ...
    {{-- Chat Window Card --}}
    <div id="chatbot-window" class="chatbot-card hidden" role="dialog" aria-modal="true" aria-labelledby="chatbot-title">
        {{-- Header --}}
        ...
        <div class="chatbot-chips">
            <button type="button" class="chat-chip" data-question="Apa rekomendasi wisata hits di Tegal & Guci?">🏖️ Wisata Hits Guci</button>
            <button type="button" class="chat-chip" data-question="Rekomendasi kuliner sate kambing & makanan khas Tegal yang wajib dicoba?">🍲 Kuliner Khas Tegal</button>
            <button type="button" class="chat-chip" data-question="Cari penginapan atau hotel bagus dekat wisata Tegal?">🏨 Penginapan & Hotel</button>
            <button type="button" class="chat-chip" data-question="Bagaimana cara sewa mobil atau motor di Tegal?">🚗 Rental Kendaraan</button>
            <button type="button" class="chat-chip" data-question="Ada event festival atau acara seru apa di Tegal?">🎪 Event & Festival</button>
        </div>
        ...
        <form id="chatbot-form" class="chatbot-input-bar">
            <input 
                ...
                id="chatbot-input" 
                class="chatbot-input" 
                placeholder="Ketik pertanyaan seputar Tegal..." 
                autocomplete="off" 
                maxlength="500" 
                required
            >
    ...
    async function sendMessage(message) {
        ...
        try {
            ...
            if (data.success && data.reply) {
                ...
                appendMessage('bot', data.reply || 'Maaf, terjadi kendala saat memproses jawaban. Silakan coba kembali.');
            }
        } catch (error) {
            console.error('Chatbot error:', error);
            typingIndicator.remove();
            appendMessage('bot', 'Mohon maaf, koneksi ke asisten AI terputus. Silakan coba beberapa saat lagi.');
        } finally {
            isProcessing = false;
