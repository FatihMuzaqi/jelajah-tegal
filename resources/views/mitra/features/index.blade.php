...
@section('page-description', 'Kelola status fitur layanan aktif dan ajukan izin pembukaan jenis layanan baru untuk usaha
    Anda.')
...
@section('content')
    ...
    <div class='stats-grid'>
        @foreach ($features as $feature)
            ...
            @php(
    $featureIcon = match ($feature->serviceType->code ?? '') {
        ...
        default => 'fa-solid fa-layer-group'
            ...
            <x-stat-card :label='$feature->serviceType->name' :value="$isActive ? 'Aktif' : 'Nonaktif'" :tone='$featureTone' :caption="$isActive ? 'Dapat dikelola' : 'Perlu aktivasi'" :icon='$featureIcon' />
        @endforeach
    </div>

    {{-- Form Pengajuan Fitur Tambahan --}}
    <x-content-card title='Ajukan Aktivasi Layanan Tambahan'
        subtitle='Pilih jenis layanan bisnis yang ingin dibuka untuk Mitra ini.' class='mt-4'>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <textarea name="reason" id="reason" rows="3" class="form-control"
                            placeholder="Contoh: Kami berencana membuka unit usaha penyewaan mobil pariwisata di Tegal..." required>{{ old('reason') }}</textarea>
                        @error('reason')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <tr>
                    <td colspan="4">
                        <x-empty-state title='Belum ada pengajuan'
                            description='Riwayat permohonan fitur tambahan akan tampil di sini.' compact />
                    </td>
                </tr>
            </tbody>
        @else
