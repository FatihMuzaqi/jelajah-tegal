    ...
    <div class='d-flex align-items-center gap-2'>
        ...
        @if(in_array($tourism->status, ['draft', 'rejected', 'published']))
            <form method='POST' action='{{ route('mitra.tourism.archive', $tourism) }}'>
                @csrf
                <button class='btn btn-sm btn-outline-danger' onclick="return confirm('Apakah Anda yakin ingin mengarsipkan destinasi ini?')">
                    Arsipkan
                </button>
            </form>
        @endif
...
<div class='row g-4'>
    ...
    <div class='col-lg-5'>
        <x-content-card title='Media & Foto Wisata'>
            <!-- Upload Form -->
            <form method='POST' enctype='multipart/form-data' action='{{ route('mitra.tourism.media', $tourism) }}' class='mb-3'>
                @csrf
                <div class='mb-2'>
                    <label class='form-label fw-semibold' style='font-size: 13px;'>Pilih File Foto</label>
                    <input class='form-control form-control-sm' type='file' name='image' accept='image/jpeg,image/png,image/webp' required onchange='previewTourismMedia(this)'>
                </div>

                <!-- Live Preview Box -->
                <div id='tourism-live-preview-box' class='mb-2 p-2 rounded' style='display: none; background: var(--lokantara-background); border: 1px dashed var(--lokantara-primary); text-align: center;'>
                    <small class='text-muted d-block mb-1'>Preview foto yang akan diunggah:</small>
                    <img id='tourism-live-preview-img' src='' style='max-height: 140px; max-width: 100%; border-radius: 6px; object-fit: cover;'>
                </div>
                        ...
                        <div class='col-6'>
                            <div class='card h-100 border' style='border-radius: 8px; overflow: hidden;'>
                                <div style='height: 100px; background: #e2e8f0; position: relative;'>
                                    <img src='{{ asset('storage/' . $media->object_key) }}' alt='{{ $media->pivot->caption ?? $tourism->name }}' style='width: 100%; height: 100%; object-fit: cover;'>
            ...
            <form method='POST' action='{{ route('mitra.tourism.hours', $tourism) }}'>
                ...
                @for($day=1; $day<=7; $day++)
                    <div class='row align-items-center mb-2 pb-2 border-bottom' style='font-size: 13px;'>
                        ...
                        <div class='col-4'>
                            <input class='form-control form-control-sm' type='time' name='hours[{{ $day }}][opens_at]' value='08:00'>
                        </div>
                        <div class='col-1 text-center text-muted'>-</div>
                        <div class='col-4'>
                            <input class='form-control form-control-sm' type='time' name='hours[{{ $day }}][closes_at]' value='17:00'>
                        </div>
                    </div>
                @endfor
    ...
    <div class='col-lg-6'>
        <x-content-card title='Paket Tiket Wisata'>
            <form method='POST' action='{{ route('mitra.tourism.packages.store', $tourism) }}' class='mb-3'>
                ...
                <div class='row g-2'>
                    <div class='col-md-5'>
                        <input class='form-control form-control-sm' name='name' placeholder='Nama Tiket (cth: Reguler)' required>
                    </div>
                    <div class='col-md-4'>
                        <input class='form-control form-control-sm' name='price' type='number' step='0.01' placeholder='Harga (Rp)' required>
                    </div>
                    <div class='col-md-3'>
                        <input class='form-control form-control-sm' name='quota_per_day' type='number' placeholder='Kuota/hari'>
                    </div>
                </div>
            ...
            @forelse($tourism->offers as $offer)
                <div class='p-3 rounded mb-2' style='background: var(--lokantara-background); border: 1px solid var(--lokantara-border); font-size: 13px;'>
                    <div class='d-flex align-items-center justify-content-between mb-2'>
                        <strong class='fs-6'>{{ $offer->name }}</strong>
                        <span class='badge bg-success'>Rp {{ number_format($offer->price, 0, ',', '.') }}</span>
                    </div>
                    
                    <form class='row g-2 align-items-center' method='POST' action='{{ route('mitra.tourism.quota', [$tourism, $offer->ticketPackage]) }}'>
                        @csrf
                        @method('PUT')
                        <div class='col-5'>
                            <input class='form-control form-control-sm' type='date' name='service_date' value='{{ date('Y-m-d') }}' required>
                        </div>
                        <div class='col-4'>
                            <input class='form-control form-control-sm' type='number' name='capacity' placeholder='Kuota' required>
                        </div>
                        <div class='col-3'>
...
<x-content-card title='Riwayat Moderasi' class='mt-4'>
    @forelse($tourism->moderationReports as $report)
        ...
        @endforeach
    @empty
        <x-empty-state title='Belum ada riwayat' description='Riwayat muncul setelah destinasi diajukan ke admin.' compact />
    @endforelse
</x-content-card>
