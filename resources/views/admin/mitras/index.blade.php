...
@section('page-description', 'Kelola tenant mitra pariwisata daerah Tegal (Dinas & Non-Dinas), aktivasi operasional, dan
    moderasi akun.')
                ...
                <tr>
                    <td colspan='6'>
                        <x-empty-state title='Tidak ada mitra ditemukan'
                            description='Silakan sesuaikan filter pencarian Anda atau buat mitra baru melalui tombol di atas.'
                            compact />
                    </td>
                </tr>
            </tbody>
                ...
                @foreach ($mitras as $mitra)
                    <tr>
                        ...
                        <td data-label='Aksi' class='text-end'>
                            ...
                            <div class='modal fade text-start' id='reset-pw-mitra-{{ $mitra->id }}' tabindex='-1'
                                aria-labelledby='resetPwMitraLabel{{ $mitra->id }}' aria-hidden='true'>
                                <div class='modal-dialog modal-dialog-centered'>
                                    <div class='modal-content border-0 shadow-lg rounded-4'>
                                        <form method='POST'
                                            action='{{ route('admin.mitras.reset-owner-password', $mitra) }}'>
                                            @csrf
                                            <div class='modal-header border-bottom py-3 px-4' style='background: #f8fafc;'>
                                                <h6 class='modal-title fw-bold text-dark'
                                                    id='resetPwMitraLabel{{ $mitra->id }}'>
                                                    <i class='fa-solid fa-key text-primary me-1'></i> Reset Password Owner
                                                    Mitra
                                                </h6>
                                            ...
                                            <div class='modal-body p-4'>
                                                <div class='p-3 rounded-3 mb-3'
                                                    ...
                                                    <div class='mb-1'><strong>Mitra:</strong> {{ $mitra->display_name }}
                                                        ({{ $mitra->category_label }})</div>
                                                    <div><strong>Akun Owner:</strong>
                                                        {{ $mitra->owner?->name ?? 'Belum ada' }} &middot;
                                                        <code>{{ $mitra->owner?->email ?? '-' }}</code></div>
                                                </div>
                                ...
                                <div class='modal-dialog'>
                                    <form class='modal-content lokantara-modal' method='POST'
                                        ...
                                        <div class='modal-header'>
                                            <h2 class='modal-title fs-5 fw-bold'>Suspend Mitra: {{ $mitra->display_name }}
                                            </h2>
                                        ...
                                        <div class='modal-body'>
                                            ...
                                            <div class='alert alert-warning fs-8 py-2 px-3 mb-3'>
                                                ...
                                                Menangguhkan mitra akan menonaktifkan visibilitas katalog dan hak akses
                                                transaksi sementara.
                                            </div>
                                            <x-textarea name='reason' label='Alasan Suspend' required
                                                placeholder='Tuliskan alasan penangguhan tenant...' />
                                        </div>
                                        <div class='modal-footer'>
                                            <button type='button' class='btn btn-outline-secondary'
                                                data-bs-dismiss='modal'>Batal</button>
                                            <button type='submit' class='btn btn-danger fw-bold'>Tangguhkan
                                                Mitra</button>
                                        </div>
                                    </form>
                                </div>
