...
<x-content-card title='Data Mitra dan Owner'>
  <form method='POST' action='{{ route('admin.mitras.store') }}'>
    ...
    <div class='row'>
      ...
      <div class='col-md-6'><x-form-input name='slug' label='Slug' hint='Huruf kecil, angka, dan tanda hubung.' required /></div>
      <div class='col-md-6'><x-select name='region_id' label='Lokasi'><option value=''>Pilih lokasi</option>@foreach($regions as $region)<option value='{{ $region->id }}' @selected(old('region_id')==$region->id)>{{ $region->name }}</option>@endforeach</x-select></div>
    </div>
