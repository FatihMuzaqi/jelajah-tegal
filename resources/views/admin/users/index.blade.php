...
@section('page-description', 'Kelola data pengguna terdaftar, status verifikasi, hak akses peran (role), dan reset kata sandi.')
    ...
    <div class='d-flex flex-column flex-md-row justify-content-between align-items-stretch align-items-md-center gap-3 mb-3'>
        ...
        <form method='GET' action='{{ route('admin.users.index') }}' class='d-flex align-items-center gap-2'>
            ...
            @if(request('q') || request('role') || request('status'))
                <a href='{{ route('admin.users.index') }}' class='btn btn-sm btn-light border text-muted' title='Reset Filter'>
    ...
    <x-table-wrapper title='Daftar Pengguna ({{ $users->total() }} Akun)'>
        @if ($users->isEmpty())
            <tbody>
                <tr>
                    <td colspan='5'>
                        <x-empty-state title='Tidak ada pengguna ditemukan' description='Silakan sesuaikan kata kunci pencarian Anda.' compact />
                    </td>
                </tr>
            </tbody>
        @else
                ...
                @foreach ($users as $u)
                    <tr>
                        ...
                        <td data-label='Aksi' class='text-end'>
                            ...
                            <div class='modal fade text-start' id='reset-user-pw-{{ $u->id }}' tabindex='-1' aria-labelledby='resetUserPwLabel{{ $u->id }}' aria-hidden='true'>
                                    ...
                                    <div class='modal-content border-0 shadow-lg rounded-4'>
                                        <form method='POST' action='{{ route('admin.users.reset-password', $u) }}'>
                                            ...
                                            <div class='modal-header border-bottom py-3 px-4' style='background: #f8fafc;'>
                                                <h6 class='modal-title fw-bold text-dark' id='resetUserPwLabel{{ $u->id }}'>
                                                    <i class='fa-solid fa-key text-primary me-1'></i> Reset Password Pengguna
                                                </h6>
                                            ...
                                            <div class='modal-body p-4'>
                                                <div class='p-3 rounded-3 mb-3' style='background: #f1f5f9; font-size: 13px;'>
                                                    ...
                                                    <div><strong>Email Akun:</strong> <code>{{ $u->email }}</code></div>
                                                </div>
                                                ...
                                                <div class='mb-3'>
                                                    ...
                                                    <input type='text' name='password' id='new_user_pwd_{{ $u->id }}' class='form-control font-monospace' placeholder='Minimal 8 karakter' required>
                                                </div>
                                                ...
                                                <div class='mb-2'>
                                                    ...
                                                    <input type='text' name='password_confirmation' id='conf_user_pwd_{{ $u->id }}' class='form-control font-monospace' placeholder='Ulangi kata sandi baru' required>
                                                </div>
                                            </div>
