...
class SaveCulinaryVenue
{
    ...
    public function execute(Mitra $mitra, array $data, $actor, ?CatalogEntity $entity=null): CatalogEntity
    {
        ...
        return DB::transaction(function() use($mitra,$service,$data,$actor,$entity){ $before=$entity?->toArray()??[]; $entity??=new CatalogEntity(['mitra_id'=>$mitra->id,'service_type_id'=>$service->id]); $entity->fill(Arr::only($data,['category_id','region_id','name','slug','description','address','is_featured'])); $entity->status=$entity->status??'draft'; $entity->save(); $entity->culinary()->updateOrCreate([],Arr::only($data,['venue_type','accepts_reservations','phone','reservation_notes'])); if(isset($data['latitude'],$data['longitude'])){ $lat = (float) $data['latitude']; $lng = (float) $data['longitude']; DB::table('catalog_locations')->updateOrInsert(['catalog_entity_id' => $entity->id], ['location' => DB::raw("ST_GeomFromText('POINT({$lat} {$lng})', 4326)"), 'latitude' => $lat, 'longitude' => $lng, 'updated_at' => now(), 'created_at' => now()]); } $entity->facilities()->sync($data['facilities']??[]); $this->audit->record($before?'culinary.updated':'culinary.created',$entity,$before,$entity->fresh()->toArray(),$actor); return $entity->fresh(['culinary','location']); });
    }
}
