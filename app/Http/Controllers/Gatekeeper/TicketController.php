...
{
    public function validateTicket(Request $request, ValidateQrTicket $action): RedirectResponse
    {
        ...
        $ticket = $action->execute(
            $data['token'],
            $request->user(),
            (string) $request->session()->get('active_mitra_id'),
            $data['device_reference'] ?? null
        );

        return back()->with('status', 'Tiket ' . $ticket->ticket_code . ' (' . ($ticket->orderItem?->item_name ?? 'Layanan Mitra') . ') berhasil divalidasi & check-in.');
    }
}
