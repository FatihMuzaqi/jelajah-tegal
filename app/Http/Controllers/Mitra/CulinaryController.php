...
{use ResolvesActiveMitra;public function index(Request $r):View{abort_unless($r->user()->can('culinary.manage'),403);$items=CatalogEntity::where('mitra_id',$this->activeMitra($r)->id)->whereHas('serviceType',fn($q)=>$q->where('code','culinary'))->latest()->paginate(15);return view('mitra.catalog-domain.index',['items'=>$items,'title'=>'Kuliner','routePrefix'=>'mitra.culinary']);}public function create(Request $r):View{abort_unless($r->user()->can('culinary.manage'),403);return view('mitra.catalog-domain.form',$this->refs()+['title'=>'Kuliner','routePrefix'=>'mitra.culinary','domain'=>'culinary']);}public function store(SaveCulinaryVenueRequest $r,SaveCulinaryVenue $a):RedirectResponse{$e=$a->execute($this->activeMitra($r),$r->validated(),$r->user());return redirect()->route('mitra.culinary.show',$e);}    public function show(Request $r, CatalogEntity $culinary): View
    {
